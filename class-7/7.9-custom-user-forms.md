...
1. In Local, click _Site folder_. As you know, this will open the site folder on your computer - in File Explorer, Finder, or another file manager.
2. Navigate to _app/public_ to access the WordPress files.
3. Inside, you'll find a _wp-content_ folder. Open that, and then open the _plugins_ folder. This is where all of the plugins for your site are stored. You can create new folders here for your custom plugins and upload plugin files here when you create them.
4. Create a folder called _geekaway-plugin_.
5. Inside, create a file named _geekaway-plugin.php_ using your favorite text or code editor.
6. Remember that when creating themes or plugins in WordPress you always place an opening PHP tag at the top of the file and never put a closing tag at the bottom. Here's the code to place in the file:
...
7. The plugin is now ready to be activated. Go to _Plugins > All Plugins_, find _geekaway-plugin_, and click _Activate_. At this point, if you make changes, all you need to do is save the changes for them to be reflected in the site. This is one of the great things about developing plugins using Local - you can make changes and see them immediately without having to do anything else.
8. Let's load the registration form and confirm that we can see the new fields.
9. Try creating an account and verify that the first name and last name fields save successfully.
10. This is cool, but what if we want to add a new user meta field that isn't a default field provided by WordPress? Let's add a _Favorite Color_ field. Because it isn't already on the _User Profile_ page, we'll need to add it there, too. Below is the final code for the plugin, but in the video I show what we need to modify rather than copying and pasting the whole thing.
...
11.  Edit your profile and confirm that you see the _Favorite Color_ field and that what you enter into the field gets saved.
12.  Edit the profile for the _instructor_ account and verify that the field appears there as well and that what you enter into the field gets saved.
13.  Go back to the registration form to confirm that all three of the new fields appear there now. Try creating a new account and filling in all of those fields. Once you've done that, check to see if the information saved to your profile.
14.  Now that the plugin is working, if you wanted to distribute this there would be one more step: packaging it for distribution. To do that, we simply zip the entire _geekaway-plugin_ folder into a file called _geekaway-plugin.zip_ and then place it somewhere outside of your WordPress installation directory for safe-keeping. That's all that we need to do to package our plugin.
15.  Now, let's test the .zip file to make sure that it works. Deactivate the plugin if it's not already deactivated and delete it from the _Plugins_ page. Don't worry - the user meta data will still be there when we reactivate the plugin, so we won't lose any of our work.
16.  Go to the _Admin area_, _Plugins > Add New Plugin_ on the _Admin menu_, and click the _Upload Plugin_ button.
17.  Click _Browse_, select the .zip file, click _Install Now_, and then _Activate_.
18.  Now check the user profile form to confirm that the _Favorite Color_ field still appears with a value.

Wait - we deactivated the plugin and then reactivated it and the custom user meta field value is still there? Yes, WordPress doesn't do any automatic clean-up when you deactivate a plugin. Meta data continues to remain in the WordPress database unless you remove it. Plugin developers (like you!) can use a hook to clean up after themselves on deactivation, but it's not uncommon for people to deactivate plugins and then reactivate them (and doing so is very easy, as you know), so the best practice is to provide a tool for cleaning up data prior to deactivation rather than assuming that anyone who deactivates your plugin wants their data removed. Imagine if you create a plugin that is installed on a site where it collects data on many thousands of users and the site owner accidentally deactivates it. They reactivate it and discover that the plugin automatically deleted all of that data. Not good! You thought you were being a good developer and not cluttering up a database, but that's likely not how they would see it. So, instead, offer a button with lots of warnings and confirmations that deletes all of your plugin's data. You can even place a link next to the _Deactivate_ link that takes people to that process in order to highlight the fact that you provide it. The _BulletProof Security_ plugin provides an example of this. We won't go into how to do that here, but I encourage you to look into it if you are interested!
