...
11. Add a block and search for _Map_. You should see a block for _WP Go Maps_. Add that block to the page and select the map you just created from the dropdown menu in the block settings. You may find that you need to adjust the map settings to get it to look the way you want. You can always go back and edit the map and the marker to make adjustments. You may also want to place it inside a _Group_ block and add a heading above it to give it some context. You can also add some text below the map if you like.
