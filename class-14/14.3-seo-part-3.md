...
So far we have been focusing on technical SEO. In this tutorial, we will begin looking at how _Yoast SEO_ helps us with on-site SEO and also touch on a few more technical SEO aspects.
...
22. Click _llms.txt_.
23. In an earlier video, we turned this feature on. This is where you can adjust the settings related to the _llms.txt_ file, which is a file that you can use to give instructions to AI bots about how to crawl your site. Currently we are having Yoast control the file contents based on the best practices that Yoast has identified, but you can also manually control it. If you want to learn more about the _llms.txt_ file, check out the [Yoast article about it](https://yoa.st/llmstxt-best-practices).
24. Click _Schema_.
25. This page allows you to adjust the default _Schema.org_ markup that Yoast includes in the head section of your pages. This is a type of structured data that search engines can use to better understand your site and provide rich snippets in search results. For example, if you have a recipe site, you can use the _Recipe_ schema type to help Google understand that your content is a recipe and then Google can provide rich snippets in search results that include things like the recipe's rating, cooking time, and more. You can use the links to learn more about the feature and you can turn it on and off, but notice that it automatically integrates with specific plugins, such as _The Events Calendar_, to include the appropriate structured data for those plugins' content. If you are using a plugin that isn't integrated with Yoast, you would use the options that we already saw under _Content types_ for any custom post types that the plugin creates to adjust the default schema types for those pieces of content. We won't make any changes here, but I encourage you to learn more about this feature and how it can help your SEO.
26. Click _Crawl optimization_.
27. There is a lot here! We're not going to go through it all, but I encourage you to [learn more about the Crawl Optimization settings using the link provided](https://yoa.st/crawl-settings). In a nutshell, this page allows us to remove from the head section and HTTP headers of our pages things that are not providing value, which reduces bandwidth (potentially speeding up the site slightly) and, probably more importantly, stops suggesting to search engine bots that they need to scan all of these additional URLs. For example, if you aren't doing anything with comment feeds, then you probably don't need to include them in the head section. Let's enable _Remove global comment feeds_.
28. Also, there's little to be gained by telling other devices the versions of PHP and WordPress that you are using and disabling these details may improve security a tiny bit, so let's enable _Remove generator tag_ and _Remove powered by HTTP header_.
29. That's all that we'll do here for now, so let's click _Save changes_.
30. Click _Breadcrumbs_.
31. Breadcrumbs are links that help people to navigate your site structure. Google sometimes uses breadcrumbs to provide context within search results pages as well. Enabling them via Yoast is a two-step process: first we configure them here and then we add the _Yoast Breadcrumbs_ block to individual pages and posts or to templates. I encourage you to [read more about breadcrumbs and their value](https://yoa.st/header-breadcrumbs) using the link displayed here. Let's scroll down and enable _Show blog page in breadcrumbs_ and _Bold the last page_.
32. Choose _Categories_ for the breadcrumbs for _Posts_.
33. Click _Save Changes_.
34. In a moment we'll add the breadcrumbs to our templates, but first let's quickly look over the rest of the settings. _Author archives_, _Date archives_, and _Format archives_ recommend that you turn off these types of archive pages but provide the typical options if you want to keep them. We are using author archives since we have multiple authors, but if you don't have multiple authors, turning off this archive type is a good idea for the reasons stated. Let's turn off _Date archives_ and _Format archives_ and then click _Save Changes_.
35. Media pages are disabled by default. You could enable them here, but for the reasons explained we will leave them off.
36. The _RSS_ page allows us to specify content that will be added to the RSS feeds. We'll leave it as-is.
37. Let's add the breadcrumbs to our templates. Go to _Appearance > Editor_.
38. Click _Templates_ and then _Single Posts_.
39. Click _Edit_.
40. Click the _Date_ block and then click _Options_ and _Add before_.
41. Type a slash and search for _Yoast_, then click _Yoast Breadcrumbs_ when it appears.
42. Save the changes to the template and then click the WordPress icon and navigate back to the _Templates_ list.
43. Click _Pages_.
44. Click _Edit_.
45. Click the _Content_ block, click _Options_, and click _Add before_.
46. We want the breadcrumbs to appear aligned with the content block, which we can do by placing the breadcrumbs block within a group block. Type a slash and search for _Group_ and then choose the default type.
47. Click the _Add block_ button and add a _Yoast Breadcrumbs_ block inside the group block.
48. Click _Save_.
