...
First of all, note that to complete these on a remote server, you should log into the server via SSH and navigate to the base folder of the WordPress installation that you want to manage using the `cd` command. Then you can run commands that will affect that WordPress installation.

Since we're using _Local_, we can use the _Site shell_ button in _Local_ to open a terminal window that is already in the base folder of our WordPress installation.
...
Perhaps I've changed the WordPress image size configuration and I need to tell WordPress to generate (or regenerate) the scaled-down versions of my uploaded images:
...
```
wp post create --post_type=page --post_title="New Page"
```
